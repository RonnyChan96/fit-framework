...
export const docPage = (div, graph, name, id) => {
   ...
   */
  self.getFrame = () => self.sm.findShapeBy(s => s.isType('docFrame'));
  ...
  const getTopSection = () => {
    ...
    const topDocSections = self.sm.getShapes(s => s.isTypeof("docSection") && s.isTopSection());
  ...
  self.getSuspensionShapes = () => {
    return self.sm.getShapes(s => s.mode && s.mode === SHAPE_IN_DOCUMENT_MODE.SUSPENSION);
  };
  ...
  self.disablePointerEvents = () => {
    const shapes = self.sm.getShapes(s => !s.isTypeof("docSection") && s.type !== "docFrame" && s.type !== "referenceVector" && !s.isLevitation);
  ...
  self.removeLevitation = () => {
    self.sm.getShapes(s => s.isLevitation).forEach(s => s.remove());
  }
