...
let topic = (id, x, y, width, height, parent) => {
  ...
  self.dragTo = position => {
    ...
    if (mind.autoAlign) {
      if (self.page.sm.getShapes().find(s => s.isType("topic") && s.container === mind.id) === self) {//该mind的第一个topic，移动mind
  ...
  self.getRoot = () => self.page.sm.getShapes().find(s => s.id === self.root);

  self.getChildren = () => self.page.sm.getShapes().filter(s => s.parent === self.id);
  ...
  self.getRelated = (shapes) => {
    ...
    const all = [self];
    !shapes && (shapes = self.page.sm.getShapes().filter(s => s.root === self.root && s.id !== self.id));
    shapes.forEach(s => {
  ...
  self.invalidate = action => {
    if (action && !action.done) {
      self.page.sm.getShapes().filter(s => s.container === self.container && s.isType("topic")).forEach(s => s.place());
      action.done = true;
