...
import { layoutCommand, shapesIndexChangedCommand, transactionCommand } from '../../core/commands.js';
...
const mind = (id, x, y, width, height, parent) => {
  ...
  self.gotoShape.moveTo = (x, y) => {
    ...
    const shapeIndexChangeCommand = (shape, before, after) => {
      ...
      arg.preIndex = before
      arg.index = after
      return shapesIndexChangedCommand(self.page, [arg]);
    }
    const appendChild = () => {
  ...
  self.getAuthor = () => self.graph.session.name;
  ...
  self.initialize = args => {
    //默认加topic
    let t = self.page.createNew("topic", x + WIDTH / 2 - 60, y + HEIGHT / 2 - 15, self.id + "$", {container: self.id, text: self.text}, self);
  };
