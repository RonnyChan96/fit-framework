...
const intervalChart = (id, x, y, width, height, parent, drawer) => {
  const chartDrawer = customizedDrawer("div", (dom, d) => {
    ...
    d.chart = new Chart({container: dom, theme: 'classic', autoFit: true});
    d.chartInstance = d.chart.interval();
  });
  ...
  self.invalidateAlone = () => {
    ...
    if (self.tag && self.tag.data && self.tag.setting) {
      ...
      //分组展示
      if (setting.dodgeX) {
        instance = self.drawer.chartInstance.data(self.tag.data).transform({type: 'dodgeX'});
      }
      // 按照某个维度排序展示
      if (setting.sort) {
        instance.transform(setting.sort)
      }
      instance.encode('x', self.tag.setting.x).encode('y', self.tag.setting.y).encode('color', 'genre');
