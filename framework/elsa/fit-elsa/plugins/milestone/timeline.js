...
let milestone = (id, x, y, width, height, parent) => {
  ...
  self.drawer.drawStatic = (context, x, y) => {
    ...
    let g1 = context.createLinearGradient(x2, y1, x + self.width, y1);
    g1.addColorStop(0, self.fontColor);
    g1.addColorStop(1, "rgba(255,255,255,0)");

    context.beginPath();
    context.moveTo(x2, y1);
    context.lineTo(x2 + self.width, y1);
    context.strokeStyle = g1;
    context.stroke();

    let y2 = self.get("vAlign") === ALIGN.TOP ? y + HEAD_HEIGHT - 10 : y + self.height - 10;
    let g2 = context.createLinearGradient(x2, y2, x2, y + self.height / 2);
    ...
    context.lineTo(x2, y + self.height / 2);
    context.stroke();
    ...
    context.lineWidth = 1;
    context.beginPath();
    context.arc(x2, y + self.height / 2, 5, 0, 2 * Math.PI);
    context.fill();
    context.stroke();

    context.fillStyle = self.fontColor;
    context.beginPath();
    context.arc(x2, y + self.height / 2, 3, 0, 2 * Math.PI);
    context.fill();
